...
Comment et quand sera appliqué la lambda dépendra de l'appelant (ici `map`).
...
* `Lazy` pour représenter une valeur qui sera produite ultérieurement, c'est une fonction sans paramètre _intéressant_ (le `()`) et contenant un type polymorphe (le `a`).
Je disais que la fonction n'avait aucun paramètre intéressant car ce qu'on veut modéliser c'est un traitement qui s'éxécutera plus tard, d'où le `Lazy`.
Concernant le `()`, on parle de tuple vide, c'est l'unique valeur du type `()`.
* `UnitTest` est un `Lazy (Bool, String)` donc un traitement qui produira une paire contenant un `Bool` et une `String`.
...
<3> Un exemple de http://lyah.haskell.fr/syntaxe-des-fonctions#filtrage-par-motif[**pattern matching**]

<4> On applique partiellement `map` avec juste une lambda, on obtient alors une fonction `[UnitTest] -> [(Bool,String)]`

<5> Le `$` est une fonction (`($) {2c} (a -> b) -> a -> b`) qui passe la valeur à sa droite à la fonction à sa gauche, c'est un moyen pratique d'éviter ceci : `(map format . runTests) tests`.
Ici on passe la valeur `tests` à la fonction composée `map format . runTests`.

<6> la fonction `:` (`(:) {2c} a -> [a] -> [a]`) ajoute une valeur au début d'une liste.

On a ici un mini-framework de test (certes limité) en quelque lignes.

== Au final, une fonction c'est vraiment juste une valeur
