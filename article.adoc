...
Partons donc à la découverte de la programmation fonctionnelle avec un language qui s'en revendique pleinement : https://www.haskell.org/[Haskell]
...
[NOTE]
====
Si on veut aller plus on peut considérer que n'importe quelle fonction est une lambda (lambda = fonction anonyme).
En l'occurrence pour Haskell la notation suivante:

[source,haskell]
----
-- double le nombre passé en paramètre
double x = x + x
----

Est juste du sucre syntaxique pour cette lambda:
[source,haskell]
----
-- idem mais avec moins de caractère
double = \x -> x + x
----
====
...
* le `return` n'est pas un mot clé mais une fonction, ici il permet de dire : _ne fais rien_, car `()` est la valeur __vide__.
====
...
[TIP]
====
Quand vous voyez des `a` et `b` dans la signature d'une fonction ce que celle-ci sera polymorphique (dans le corps de la fonction ce sera une valeur).
...
On a vu que dans un language fonctionnel, une fonction est une valeur comme une autre que l'on peut aisément manipuler. 
