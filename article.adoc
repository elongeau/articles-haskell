...
Haskell permet de __placer__ une fonction soit avant ses paramètres (prefixe) ou entre (infixe). 
Les fonctions alphanumériques seront prefixe et celles dont le nom est un ou des symboles (`.`, `>>`) seront infixe.
On peut intervertir leurs usages:

[cols="1,1a,1a"]
|===
||en prefixe | en infeix
|fonction __prefixe__
|
[source,haskell]
----
add 1 2
...
On a vu comment faire une lambda à un paramètre et pour plusieurs paramètres ?
On fait comme ceci : 
...
Plus de paramètres !
Rien de magique la dedans, encore moins une obscure fonctionnalité d'Haskell, quelques explications: 
