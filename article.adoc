...
Parce qu'en notation mathématique la composition s'écrit `rond`
====
...
<1> On défini deux **type alias** :

* `Lazy` pour représenter une fonction sans paramètre _intéressant_ (le `()`, voir la note ci-dessous) et contenant un type polymorphe (le `a`).
...
.le cas `()`
====
Je disais que la fonction n'avait aucun paramètre intéressant car ce qu'on veut modéliser un traitement qui s'éxécutera plus tard, d'où le `Lazy`.
Concernant le `()`, on parle de tuple vide, c'est l'unique valeur du type `()`.
====

[TIP]
.Postfix et infix (à supprimer ?)
====
Quand une fonction à deux paramètres on peut l'utiliser de deux manières différente:

postfix::
le nom de la fonction est en premier suivi des ces paramètres, c'est ce qu'on a vu jusqu'a présent: `add 1 1`.
C'est la notation privilégiée pour les fonctions dont le nom est alphanumérique

infix::
le nom de la fonction est placé entre ces paramètres comme un opérateur : ``1 + 1`` (oui `+` est une fonction).
Utilisé plus particuièrement pour les fonctions dont le nom est un symbole (`?` dans notre cas).

On peut intervertir les usage en entourant une fonction postfixe de `{bq}` : ``1 {bq}add{bq} 1``, pour une fonction infixe ce sera avec des parenthèse : `(+) 1 1`
====
...
== Une fonction c'est vraiment juste une valeur

Histoire d'enfoncer le clou, je voudrais vous montrer un petit exemple de manipulation de fonction.
On construit d'abord une liste contenant des fonctions:
...
Ensuite on utilise le tout:
[source,haskell]
----
λ> map (\f -> f "world") listOfFunction
[ "dlrow"
, "Hello world"
]
----

Pour rappel, `map` prend en paramètre une liste et applique une fonction à chaque élément de la dite liste.
Dans notre cas la fonction est la lambda `\f -> f "world"`,
les éléments de la liste étant des fonctions, `f` en est une également, on l'applique alors sur `world`.
