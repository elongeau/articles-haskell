...
== Un exemple complet (titre temporaire !!!)
...
.le `()`
====
`()` représente soit un type soit la seule valeur de ce type: le tuple vide, c'est à dure avec aucune valeur.
Dans notre cas il permet dire qu'une fonction n'a aucun paramètre intéressant, car techniquement elle en a un: `()`.
====
