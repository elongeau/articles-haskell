...
Plusieurs éléments de syntaxe sont à l'oeuvre ici, on va d'abord passer rapidement sur eux, dans l'ordre : 

* `Show b` : pour dire que la valeur de type `b` sera convertible en `String`
* `(a -> b)` : une fonction polymorphique de `a` vers `b`, on l'entoure de parenthèse pour indiquer qu'il s'agit d'une fonction et non pas de deux paramètres distincts
